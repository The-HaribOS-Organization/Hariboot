...
config_file_t *parseConfigFile(EFI_SYSTEM_TABLE *SystemTable, UINT8 *configArray) {
    ...
    cFile->hasBackground = (configArray[20] - 0x30);
    cFile->width = ((configArray[22] - 0x30) * 1000) + ((configArray[23] - 0x30) * 100) + ((configArray[24] - 0x30) * 10) + (configArray[25] - 0x30);
    cFile->height = ((configArray[27] - 0x30) * 1000) + ((configArray[28] - 0x30) * 100) + ((configArray[29] - 0x30) * 10) + (configArray[30] - 0x30);


    cFile->background_color[0] = (Vec3){
    ...
    cFile->loadingBarFrontColor = (Vec3){
        ...
        ((configArray[132] - 0x30) * 100) + ((configArray[133] - 0x30) * 10) + (configArray[134] - 0x30),
        ((configArray[136] - 0x30) * 100) + ((configArray[137] - 0x30) * 10) + (configArray[138] - 0x30),
        ((configArray[140] - 0x30) * 100) + ((configArray[141] - 0x30) * 10) + (configArray[142] - 0x30)
    };

    cFile->loadingBarBackColor = (Vec3){
        ((configArray[144] - 0x30) * 100) + ((configArray[145] - 0x30) * 10) + (configArray[146] - 0x30),
        ((configArray[148] - 0x30) * 100) + ((configArray[149] - 0x30) * 10) + (configArray[150] - 0x30),
        ((configArray[152] - 0x30) * 100) + ((configArray[153] - 0x30) * 10) + (configArray[154] - 0x30),
        ((configArray[156] - 0x30) * 100) + ((configArray[157] - 0x30) * 10) + (configArray[158] - 0x30)
    };
